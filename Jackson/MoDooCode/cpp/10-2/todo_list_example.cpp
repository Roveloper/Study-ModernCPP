...
class Todo {
  ...
  public:
    ...
    bool operator<(const Todo& rhs) const { // set을 사용하기 위해서는 operator<가 꼭 정의가 되어 있어야함 (그리고 무조건 const ref를 받는 const함수여야 함)
      if (priority_ == rhs.priority_) { // 이를 나눠서 구현하지 않으면, 중요도가 같은 애들은 하나 밖에 정의할 수 없게됩니다. (a < b도 false, b < a도 false이면 중복으로 판단)
